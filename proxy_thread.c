/*
 * proxy_thread.c - A Concurrent Web Proxy using Threads
 *
 *
 ...
 *
 * This proxy uses threads. It acts very similarly to proxy.c, but it instead
 * creates a pthread and calls a thread function to process a request.
 *
 * This proxy is thread-safe by using a pthread_mutex_t. The wrappers for this
 * mutex can be found in csapp.c under the Semaphore functions.
 *
 * This very simply creates a new thread whenever a new request is accepted, then
 * processes this request on one thread.
 *
 * Since this program is run on only one process, we can use a mutex to lock the
 * process whenever something thread-unsafe occurs, such as writing to the log file.
 */
...
/* Check arguments */
  ...
  while (1) {
    ...
    Pthread_create(&tid, NULL, process_request_thread_function, targ);
...
void process_request(int connfd, struct sockaddr_in clientaddr) {
  ...
  // get thread id for printing status of bytes sent
  uint64_t tid;
...
#if defined(DEBUG)
    printf("Forwarded %d bytes from end server to client. Thread %lu\n", n,
           tid);
    fflush(stdout);
#endif
    bzero(buf, MAXLINE);
